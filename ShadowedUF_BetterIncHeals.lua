...
local IncHeal = ShadowUF.IncHeal
local frames = {}
...
function IncHeal:OnLayoutApplied(frame)
	if (frame.visibility.incHeal and frame.visibility.healthBar) then
		...
		frame.incHeal:SetStatusBarColor(ShadowUF.db.profile.healthColors.inc.r, ShadowUF.db.profile.healthColors.inc.g,
			ShadowUF.db.profile.healthColors.inc.b, ShadowUF.db.profile.bars.alpha)
		...
		if ((ShadowUF.db.profile.units[frame.unitType].healthBar.invert and ShadowUF.db.profile.bars.backgroundAlpha == 0) or (not ShadowUF.db.profile.units[frame.unitType].healthBar.invert and ShadowUF.db.profile.bars.alpha == 1)) then
			...
			frame.incHeal.cappedWidth = frame.incHeal.healthWidth *
			(ShadowUF.db.profile.units[frame.unitType].incHeal.cap - 1)
		end
	end
end
...
local function updateHealthBar(frame, interrupted)
	...
	local healed = (HealComm:GetHealAmount(frame.unitGUID, HealComm.ALL_HEALS, time + timeBand) or 0) *
	HealComm:GetHealModifier(frame.unitGUID)
	...
	if (frame.visibility.incHeal and frame.visibility.healthBar) then
		if (healed > 0) then
			...
			if (frame.incHeal.simple) then
				...
				frame.incHeal:SetMinMaxValues(0,
					UnitHealthMax(frame.unit) * ShadowUF.db.profile.units[frame.unitType].incHeal.cap)
				...
				frame.incHeal:SetPoint("TOPLEFT", SUFUnitplayer, "TOPLEFT", frame.incHeal.healthX + healthWidth,
					frame.incHeal.healthY)
			end
		else
